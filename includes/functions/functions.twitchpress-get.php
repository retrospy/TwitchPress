...
function twitchpress_get_users_subscription_status( $wp_user_id ) {
    return get_user_meta( $wp_user_id, 'twitchpress_substatus_mainchannel', true );    
}

/**
* Return WP users sub plan name as named by channel owner...
* 
* Do not mistake for the Twitch.tv sub plan value itself...
* 
* @param mixed $wp_user_id
* 
* @version 1.0
*/
function twitchpress_get_users_sub_plan_name( $wp_user_id ) {
    $channel_id = twitchpress_get_main_channels_twitchid();

    return get_user_meta( $wp_user_id, 'twitchpress_sub_plan_name_' . $channel_id, true );
}

function twitchpress_get_users_follower_status( $wp_user_id ) {
    return get_user_meta( $wp_user_id, 'twitchpress_follower_status', true );
}