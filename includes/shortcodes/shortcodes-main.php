...
function twitchpress_shortcode_init( $atts, $content = null ) {         
    ...
    ), $atts, 'twitchpress_shortcodes_advanced_' . $atts['shortcode'] );
...
function twitchpress_shortcode_embed_everything( $atts ) {     
    ...
    $atts = shortcode_atts( array(          
        ...
        'defaultcontent'  => false, // change to videos, collection  
        'videoid'         => 1040648073, // This default is a Fortnite video             
    ), $atts, 'twitchpress_embed_everything' );

    // Establish channel ID when only the channel name has been provided...
    if( isset( $atts['channel'] ) && !isset( $atts['channel_id'] ) ) {
        $twitch_api = new TwitchPress_Twitch_API();
        ...
        switch ( $atts['defaultcontent'] ) {
           case 'videos':
                ...
                <div id="twitchpress-embed-everything">
                    <iframe
                        src="https://player.twitch.tv/?channel=' . $atts['channel'] . '&parent=' . $_SERVER['HTTP_HOST'] . '&muted=true"
                        height="720"
                        width="1280"
           ...
           case 'video':
                ...
                <div id="twitchpress-embed-everything">
                    <iframe
                        src="https://player.twitch.tv/?video=' . $atts['videoid'] . '&parent=' . $_SERVER['HTTP_HOST'] . '&autoplay=false"
                        height="720"
                        width="1280"
                        allowfullscreen="true">
                    </iframe>        
           ...
           case 'collection':
                ...
                <div id="twitchpress-embed-everything">
                    <iframe
                        src="https://player.twitch.tv/?collection=' . $atts['collectionid'] . '&parent=' . $_SERVER['HTTP_HOST'] . '"
                        height="720"
                        width="1280"
                        allowfullscreen="true">
                    </iframe>        
...
/**
* Shortcode for channel status line...
* 
* @param mixed $atts
* 
* @version 2.0
...
function twitchpress_channel_status_line_helix( $atts ) {      
    ...
    set_transient( 'twitchpress_channel_status_line' . $atts['channel_id'], $html_output, 120 );
...
function twitchpress_shortcode_channel_list( $atts ) {                                 
    require_once( TWITCHPRESS_PLUGIN_DIR_PATH . 'includes/shortcodes/channellist/twitchpress-shortcode-channellist.php' );    
...
function twitchpress_shortcode_clips_gallery( $atts ) { 
    require_once( TWITCHPRESS_PLUGIN_DIR_PATH . 'includes/shortcodes/clipsgallery/twitchpress-shortcode-clipsgallery.php' );    
...
function twitchpress_shortcode_streamlabs( $atts ) { 
    require_once( TWITCHPRESS_PLUGIN_DIR_PATH . 'includes/shortcodes/streamlabs/twitchpress-shortcode-streamlabs.php' );    
...
function twitchpress_shortcode_sub_only_content( $atts, $content = null ) { 
    $atts = shortcode_atts( array(  
        'id'            => 'twitchpressbutton',           
        'text'          => __( 'Connect to Twitch', 'twitchpress' ),
        'require_login' => 'no',
    ...
    $shortcode_object->atts = $atts;
    $shortcode_object->sub_only_content = $content; // Posts actual content may be blocked for visitor    
    return $shortcode_object->output();    
}

function twitchpress_shortcode_team_roster( $atts, $content ) {
